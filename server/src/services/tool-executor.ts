...
async function executeInventoryQuery(
  ...
  try {
    ...
    if (formatted.length > 0) {
      let baseHint =
        formatted.length > 1
          ? `ESTOQUE ATUAL (${formatted.length} veículo(s)). Cada veículo tem seu próprio bloco "Fotos do veículo ... (id: ...)" abaixo. NÃO inclua fotos agora. Liste APENAS dados em texto (modelo, ano, km, preço, cor) e pergunte se o cliente quer ver fotos. PROIBIDO usar frases como "Dá uma olhada nas fotos!" ou similares nesta resposta — essas frases só cabem QUANDO você estiver de fato enviando as fotos. Quando o cliente PEDIR ou ACEITAR ver fotos de UM veículo, inclua na sua resposta APENAS o bloco de fotos DESSE veículo (o que tiver o id indicado em ENVIAR_FOTOS_VEICULO: nome | id: uuid) e a linha ENVIAR_FOTOS_VEICULO. Nunca inclua fotos de outros veículos.`
          : `ESTOQUE ATUAL (${formatted.length} veículo(s)). Fotos disponíveis em photos_markdown — NÃO inclua fotos agora. Liste APENAS dados em texto (modelo, ano, km, preço, cor) e pergunte se o cliente quer ver fotos (ex: "Quer ver as fotos dele?"). PROIBIDO usar frases como "Dá uma olhada nas fotos!" ou similares nesta resposta — essas frases só cabem QUANDO você estiver de fato enviando as fotos. Quando o cliente PEDIR ou ACEITAR ver fotos, aí sim inclua o conteúdo de photos_markdown na resposta junto com ENVIAR_FOTOS_VEICULO.`;
      if (hasVideoDetails) {
...
async function executeArtaxnetAvailability(
  supabase: ReturnType<typeof createNexusClient>,
  tool: ToolDef,
  args: Record<string, unknown>,
  agentId: string
  ...
  try {
    ...
    const checkOut = args.checkOut ?? args.check_out ?? args.CheckOut;
    if (checkIn == null || checkOut == null || String(checkIn).trim() === "" || String(checkOut).trim() === "") {
      return { success: false, result: null, error: "Parâmetros check_in e check_out são obrigatórios." };
    }
    const data = await runArtaxnetAvailabilityQuery(
      {
        checkIn: String(checkIn),
        checkOut: String(checkOut),
        adults: args.adults != null ? Number(args.adults) : undefined,
        children: args.children != null ? Number(args.children) : args.kids != null ? Number(args.kids) : undefined,
        coupon: args.coupon != null ? String(args.coupon) : undefined,
      },
      (tool.execution_config || null) as Record<string, unknown> | null
    );
    ...
    return {
      ...
      result: {
        ...
        children: data.children,
        rooms: data.rooms,
        otaComparisonEnabled: data.otaComparisonEnabled,
        otas: data.otas,
      },
    };
  } catch (e: unknown) {
    const err = e as Error;
    return { success: false, result: null, error: err?.message || "Consulta Artaxnet falhou" };
  }
}
  ...
  try {
    ...
    if (action === "criar" || action === "create") {
      ...
      if (conflicting && conflicting.length > 0) {
        return {
          success: false,
          result: null,
          error: "Hor├írio j├í ocupado. Use check_availability para ver slots livres.",
        };
      }
    ...
    if (action === "listar_eventos" || action === "list_events") {
      ...
      if (!events || events.length === 0) {
        ...
        // (mesmo padr├úo usado pela action "cancelar")
...
async function executeNearestUnit(args: Record<string, unknown>): Promise<ToolExecutionResult> {
  try {
    ...
    if (!cep) {
      return { success: false, result: null, error: "CEP ├® obrigat├│rio" };
    }
  ...
  try {
    ...
    if (!assignResp.ok) {
      ...
      return {
        success: false,
        result: null,
        error: `Falha na atribuição Chatwoot: ${assignResp.status} — ${detail.slice(0, 200)}`,
      };
    }
...
export async function executeTool(
  ...
  switch (tool.tool_type) {
    ...
    case "artaxnet_availability":
      return executeArtaxnetAvailability(supabase, tool, args, agentId);
...
async function executeSendNotification(
  ...
  if (!success) {
    return { success: false, result: null, error: error || "Falha ao enviar notifica├º├úo" };
  }
  return { success: true, result: { message: "Notifica├º├úo enviada com sucesso" } };
}
  ...
  try {
    ...
    if (!query) {
      return {
        success: false,
        result: null,
        error: "Par├ómetro 'query' ├® obrigat├│rio para busca RAG",
      };
    }
...
/** Palavras s├│ de inten├º├úo gen├®rica ("fotos das su├¡tes", "manda quartos") ÔåÆ sem filtro ILIKE; listar cat├ílogo. */
const BROAD_GALLERY_TOKENS = new Set(
  [
    ...
    "galeria", "album", "categorias", "categoria", "tipo", "tipos", "nos", "n├│s", "vc", "voce", "voces", "voc├¬s", "o", "os",
