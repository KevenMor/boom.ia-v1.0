import type { AgentMirrorPayload } from "./chatwoot-agent-mirror.js";
...
<head>
  ...
  <style>
    :root{--bg:#0f172a;--card:#1e293b;--card2:#263245;--border:#334155;--text:#f1f5f9;--muted:#94a3b8;--brand:#3b82f6;--success:#22c55e;--warn:#f59e0b;--err:#ef4444;}
    *{box-sizing:border-box;margin:0;padding:0;}
    body{font:14px/1.5 system-ui,sans-serif;background:var(--bg);color:var(--text);min-height:100dvh;}
    header{position:sticky;top:0;z-index:50;background:var(--card);border-bottom:1px solid var(--border);padding:10px 16px;display:flex;align-items:center;justify-content:space-between;gap:10px;}
    ...
    .active{background:#166534;color:#bbf7d0;}.test{background:#854d0e;color:#fef08a;}.inactive{background:#374151;color:#9ca3af;}
    ...
    .tab{padding:8px 14px;border:0;background:none;color:var(--muted);cursor:pointer;font:inherit;font-size:13px;border-bottom:2px solid transparent;white-space:nowrap;}
    ...
    input,select,textarea{width:100%;padding:8px 10px;border-radius:6px;border:1px solid var(--border);background:var(--card2);color:var(--text);font:inherit;font-size:13px;outline:none;}
    input:focus,select:focus,textarea:focus{border-color:var(--brand);}
    textarea{resize:vertical;min-height:100px;}
    .slider-row{display:flex;align-items:center;gap:10px;}
    input[type=range]{flex:1;accent-color:var(--brand);}
    .slider-val{min-width:40px;text-align:right;font-size:12px;color:var(--muted);}
    .status-group{display:flex;flex-direction:column;gap:0;border:1px solid var(--border);border-radius:8px;overflow:hidden;}
    .status-opt{display:flex;gap:10px;align-items:flex-start;padding:10px 12px;cursor:pointer;border-bottom:1px solid var(--border);}
    .status-opt:last-child{border-bottom:0;}
    .status-opt:hover{background:var(--card2);}
    .status-opt.sel{background:#1e3a5f;}
    .status-opt input{width:auto;flex-shrink:0;margin-top:3px;}
    .status-opt .title{font-size:13px;font-weight:600;}
    .status-opt .desc{font-size:11px;color:var(--muted);}
    .tool-item{border:1px solid var(--border);border-radius:6px;padding:8px 10px;margin-top:8px;}
    ...
    .btn{padding:8px 18px;border-radius:6px;border:0;font:inherit;font-size:13px;font-weight:600;cursor:pointer;}
    ...
    .section-title{font-size:13px;font-weight:700;margin-bottom:14px;color:var(--text);}
    ...
    .prompt-pre{width:100%;padding:10px;border-radius:6px;border:1px solid var(--border);background:var(--card2);color:var(--text);font:13px/1.5 monospace;resize:vertical;min-height:140px;}
    ...
    .warn-box{background:#422006;border:1px solid #92400e;border-radius:6px;padding:10px 12px;font-size:12px;color:#fde68a;margin-bottom:10px;}
  </style>
</head>
<body>
<header>
  <div class="logo">
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 1 0-16 0"/><path d="M12 12v9"/></svg>
    Boom IA — Agente
  </div>
  <div class="save-bar">
    <span id="acct-label" style="font-size:11px;color:var(--muted)">Conta Chatwoot ${JSON.stringify(accountId)}</span>
    ...
    <div id="panel-basic" class="tab-panel active">
      <div class="card">
        ...
        <div class="grid">
          <div>
            <label>Nome</label>
            <input id="f-name" oninput="markDirty()"/>
          </div>
          <div>
            <label>Descrição</label>
            <input id="f-desc" oninput="markDirty()"/>
          </div>
        </div>
      ...
      <div class="card">
        ...
        <div class="grid">
          <div>
            ...
            <select id="f-provider" onchange="onProviderChange();markDirty()"></select>
          </div>
          <div>
            <label>Modelo</label>
            <input id="f-model" oninput="markDirty()" placeholder="ex: gemini-2.5-flash-lite"/>
          </div>
        </div>
        <div style="margin-top:14px">
        ...
        <div id="prompt-registry-note" style="display:none" class="warn-box" style="margin-top:14px"></div>
        <div style="margin-top:14px">
          <label>Prompt em produção (preview)</label>
          <textarea id="f-prompt-preview" class="prompt-pre" readonly style="min-height:140px;opacity:.7"></textarea>
        </div>
        <div style="margin-top:14px">
      ...
      <div class="card">
        ...
        <div class="grid">
          <div style="grid-column:span 2">
            ...
            <input id="f-cw-url" oninput="markDirty()" placeholder="https://mega.atendai.app"/>
          </div>
          <div>
            <label>Account ID</label>
            <input id="f-cw-acct" oninput="markDirty()"/>
          </div>
          <div>
            <label>API Token</label>
            <input id="f-cw-token" type="password" oninput="markDirty()" placeholder="••••"/>
          </div>
        </div>
      </div>
    ...
    <div id="panel-sched" class="tab-panel">
      <div class="card">
        ...
        <div style="margin-top:10px">
          ...
          <input id="f-offline-msg" oninput="markDirty()" placeholder="Fora do horário de atendimento."/>
        </div>
      </div>
      <div class="card" style="margin-top:0">
        <div class="toggle-wrap">
        ...
        <div style="margin-top:10px">
          <label>Intervalos de follow-up (minutos, separados por vírgula)</label>
          <input id="f-fu-intervals" oninput="markDirty()" placeholder="10,20,30"/>
        </div>
        <div class="grid" style="margin-top:10px">
          <div>
            <label>Silêncio — início</label>
            <input type="time" id="f-fu-quiet-start" oninput="markDirty()"/>
          </div>
          <div>
            <label>Silêncio — fim</label>
            <input type="time" id="f-fu-quiet-end" oninput="markDirty()"/>
          </div>
        </div>
      </div>
    </div>
...
<script>
var STATE = { agent: null, providers: [], dirty: false };
var KEY = ${safeKey};
...
function setStatus(v) {
  ["active","test","inactive"].forEach(function(s) {
    ...
    if (el) el.classList.toggle("sel", s === v);
    var inp = el ? el.querySelector("input") : null;
    if (inp) inp.checked = s === v;
  });
  STATE.currentStatus = v;
  markDirty();
}

function onProviderChange() {
  // providers list is static for now — model typed manually
}
...
function fillForm(agent) {
  ...
  if (agent.avatar_url) { av.src = agent.avatar_url; av.style.display = "block"; }
  else av.style.display = "none";

  var badgeHtml = '<span class="badge ' + (agent.status || "inactive") + '">' +
    (agent.status === "active" ? "Ativo" : agent.status === "test" ? "Teste" : "Inativo") + "</span>";
  document.getElementById("acct-label").innerHTML =
    badgeHtml + ' &nbsp; Conta ' + esc(ACCOUNT_ID);

  document.getElementById("f-name").value = agent.name || "";
  document.getElementById("f-desc").value = agent.description || "";
  setStatus(agent.status || "inactive");
  STATE.currentStatus = agent.status || "inactive";

  var provSel = document.getElementById("f-provider");
  provSel.innerHTML = "";
  STATE.providers.forEach(function(p) {
    var o = document.createElement("option");
    o.value = p.id; o.textContent = p.name;
    if (p.id === agent.provider_id) o.selected = true;
  ...
  document.getElementById("lbl-temp").textContent = parseFloat(temp).toFixed(2);

  var topp = cfg("top_p", 0.8);
  document.getElementById("f-topp").value = topp;
  document.getElementById("lbl-topp").textContent = parseFloat(topp).toFixed(2);

  var topk = cfg("top_k", 40);
  document.getElementById("f-topk").value = topk;
  document.getElementById("lbl-topk").textContent = topk;

  document.getElementById("f-system-prompt").value = agent.system_prompt || "";

  if (agent.prompt && agent.prompt.uses_registry) {
    var note = document.getElementById("prompt-registry-note");
    note.style.display = "block";
    note.textContent = "Prompt principal no código: " + (agent.prompt.slug || "") +
      " v" + (agent.prompt.version || "?") + ". O campo abaixo é complementar.";
  ...
  var intervals = cfg("followup_intervals", [10,20,30]);
  document.getElementById("f-fu-intervals").value = Array.isArray(intervals) ? intervals.join(",") : String(intervals);
  ...
  if (agent.tools && agent.tools.length) {
    toolsEl.innerHTML = agent.tools.map(function(t) {
      return '<div class="tool-item"><span class="tool-name">' + esc(t.name) +
        '</span><span class="tool-type">' + esc(t.tool_type) + "</span>" +
        (t.description ? '<div class="tool-desc">' + esc(t.description) + "</div>" : "") + "</div>";
    }).join("");
  }
}

function buildPayload() {
  var intervals = document.getElementById("f-fu-intervals").value
    .split(",").map(function(x){ return parseInt(x.trim(),10); }).filter(function(n){ return !isNaN(n); });
  var currentConfig = STATE.agent ? (STATE.agent.config || {}) : {};
  return {
    ...
    config: Object.assign({}, currentConfig, {
      ...
      followup_max_attempts: intervals.length,
...
function saveAgent() {
  ...
  var url = API_BASE + "/embed/chatwoot/agents/" + encodeURIComponent(STATE.agent.id) +
    "?account_id=" + encodeURIComponent(ACCOUNT_ID) + "&key=" + encodeURIComponent(KEY);

  fetch(url, {
    method: "PATCH",
    headers: { "Content-Type": "application/json", "x-chatwoot-mirror-key": KEY },
    ...
    .then(function(res) {
      ...
      if (!res.ok) {
        btn.disabled = false;
        errEl.textContent = res.j.error || "Erro ao salvar";
        return;
      }
      STATE.dirty = false;
...
function load() {
  if (!KEY || !ACCOUNT_ID) {
    document.getElementById("loading").innerHTML =
      '<div class="alert alert-err">key ou account_id ausente na URL.</div>';
    return;
  }
  var url = API_BASE + "/embed/chatwoot/agents?account_id=" +
    encodeURIComponent(ACCOUNT_ID) + "&key=" + encodeURIComponent(KEY);
  fetch(url, { headers: { "x-chatwoot-mirror-key": KEY } })
    .then(function(r) { return r.json().then(function(j){ return { ok: r.ok, j: j }; }); })
    .then(function(res) {
      document.getElementById("loading").style.display = "none";
      if (!res.ok) {
        showAlert(res.j.error || "Erro " + res.status, "err");
        return;
      }
      var agents = res.j.agents || [];
      STATE.providers = res.j.providers || [];
      if (!agents.length) {
        showAlert(res.j.message || "Nenhum agente vinculado a esta conta.", "warn");
        return;
      }
    ...
    .catch(function(e) {
      document.getElementById("loading").innerHTML = '<div class="alert alert-err">' + esc(e.message) + "</div>";
    });
}
