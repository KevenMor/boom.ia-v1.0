...
function renderLoteamentosEmbedViewHtml(
  frontendBase: string,
  embedKey: string,
  accountId: string,
  theme: "dark" | "light",
...
<body data-theme="${theme}">
  <div class="loading" id="msg">Carregando gestão de lotes…</div>
  <script>
    (function(){
      ...
      var target=frontend+"/embed/chatwoot/loteamentos#"+hash.toString();
...
export async function embedChatwootLoteamentosRoutes(fastify: FastifyInstance) {
  fastify.get(
    "/embed/chatwoot/loteamentos/view",
    async (
      ...
      return reply
        ...
        .send(renderLoteamentosEmbedViewHtml(publicFrontendBase(), key, accountId, theme));
    },
  );

  fastify.get(
    "/embed/chatwoot/loteamentos/bootstrap",
  ...
  fastify.route({
    ...
    url: "/embed/chatwoot/loteamentos/*",
    handler: async (
      req: FastifyRequest<{
        Params: { "*": string };
      ...
      const targetUrl = `/api/loteamentos/${subPath}${qs.toString() ? `?${qs.toString()}` : ""}`;
      const headers: Record<string, string> = {
        "x-nexus-embed-loteamentos": embedKey,
