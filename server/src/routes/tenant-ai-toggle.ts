  ...
  fastify.get(
    ...
    async (req: FastifyRequest<{ Querystring: { tenant_id?: string } }>, reply: FastifyReply) => {
      ...
      const { data: tenant, error: tErr } = await supabase
        ...
        .maybeSingle();
      if (tErr) {
        fastify.log.error({ err: tErr }, "[tenant-ai] erro ao buscar tenant");
      ...
      if (aErr) {
        fastify.log.error({ err: aErr }, "[tenant-ai] erro ao buscar agents");
      ...
      fastify.log.info({ tenantId, total, activeCount }, "[tenant-ai] status");
      return reply.send({
        tenant_id: tenantId,
        agents_total: total,
  ...
  fastify.post(
    ...
    async (
      ...
      const { data: updated, error: uErr } = await supabase
        ...
        .select("id, status");
      if (uErr) {
        fastify.log.error({ err: uErr }, "[tenant-ai] erro ao atualizar agents");
        return reply.status(500).send({ error: uErr.message });
      }
      const list = updated ?? [];
      const total = list.length;
      const activeCount = list.filter((r) => r.status === "active").length;
      fastify.log.info({ tenantId, newStatus, total }, "[tenant-ai] toggle ok");
      return reply.send({
        ok: true,
        tenant_id: tenantId,
        updated_count: total,
