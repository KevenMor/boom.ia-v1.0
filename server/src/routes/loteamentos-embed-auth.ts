...
export function getEmbedLoteamentosTenantId(req: FastifyRequest): string | null {
  const expected = (process.env.CHATWOOT_MIRROR_EMBED_KEY || "").trim();
  if (!expected) return null;

  const headerKey = req.headers["x-nexus-embed-loteamentos"];
...
export async function authorizeLoteamentos(
  req: FastifyRequest,
  reply: FastifyReply,
  ...
  const embedTenant = getEmbedLoteamentosTenantId(req);
  if (embedTenant) {
